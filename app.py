...
# Define MirNet model architecture
# ...
...
def enhance_image(original_image):
    ...
    enhanced_image = enhanced_image.clip(0, 1)  # Clip values to range [0, 1]
    return enhanced_image

def main():
    st.title('Night-time Image Enhancement with MirNet')
    
    uploaded_image = st.file_uploader("Upload an image...", type=["jpg", "jpeg", "png"])

    if uploaded_image is not None:
        weights_path = "LLIE.h5"  # Provide the path to your trained model weights
        ...
        enhanced_image = Image.fromarray((enhanced_image[0] * 255).astype(np.uint8))
